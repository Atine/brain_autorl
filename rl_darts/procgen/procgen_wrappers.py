...
def vector_wrap_environment(venv,
  ...
  if normalize_obs or normalize_ret:
    venv = vec_env.VecNormalize(
        venv=venv, norm_obs=normalize_obs, clip_obs=10., clip_reward=10.)
  return venv
...
class AcmeProcgenEnv(acme_gym_wrapper.GymWrapper):
  ...
  def __init__(self, env_name, seed=0, distribution_mode="hard", **kwargs):
    ...
    def make_env(rand_seed):
      ...
      output_env = Scalarize(output_env)
      return output_env
...
class TFAgentsParallelProcGenEnv(tf_agents_environments.py_environment.PyEnvironment):
...
class CustomMultiGameProcGenEnv(tf_agents_environments.py_environment.PyEnvironment):
